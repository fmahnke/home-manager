...
let
  ...
  mtaSection = account:
    with account;
    let
      passCmd = if passwordCommand == null then
        null
      else
        escapeShellArgs passwordCommand;
    in if neomutt.sendMailCommand != null then {
      ...
      in {
        smtp_url = if smtp == null then null else "'${smtpBaseUrl}'";
        smtp_pass = if passCmd == null then null else ''"`${passCmd}`"'';
      };
  ...
  accountStr = account:
    ...
    in ''
      ...
      set ssl_force_tls = ${
        lib.hm.booleans.yesNo (if smtp == null then
          false
        else
          (smtp.tls.enable || smtp.tls.useStartTls))
      }
